...
function Feed() {
  ...
  async function load() {
    try {
      setPosts(await fetchApprovedFeed(user?.id || null));
    } catch {
      setPosts([]);
    }
  }
        ...
        <div className="flex items-center justify-between gap-3 mb-5 sm:mb-6">
          <div className="min-w-0">
            <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-primary font-semibold mb-1">
              <Newspaper className="w-3.5 h-3.5" /> Համայնք
            </div>
            <h1 className="font-display text-2xl sm:text-3xl font-bold leading-tight break-words">
              Համայնքի ֆիդ
            </h1>
            <p className="text-sm text-muted-foreground mt-1">
              Հաստատված գրառումներ՝ ուսանողներից։
            </p>
          </div>
          <Link
            to="/feed/create"
            className="shrink-0 inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold hover:opacity-90 min-h-[44px]"
          >
        ...
        {posts === null ? (
          ...
          <div className="text-center py-14 px-4 card-base rounded-2xl animate-rise">
            ...
            <h3 className="font-display text-base font-semibold mb-1">
              Դեռ հաստատված գրառումներ չկան։
            </h3>
            <p className="text-sm text-muted-foreground">Հաստատված գրառումները կհայտնվեն այստեղ։</p>
            <Link
              to="/feed/create"
