...
import { Trophy, Rocket, Calendar, Users, Sparkles, type LucideIcon } from "lucide-react";
...
type Item = {
  id: string;
  type: "achievement" | "project" | "participation";
  title: string;
  sub?: string;
  at: string;
  icon: LucideIcon;
    ...
    (async () => {
      ...
      const feed: Item[] = [
        ...(ach || []).map((achievement) => ({
          ...
          title: `Ստացար «${achievement.badge}» նշանը`,
          at: achievement.earned_at,
          icon: Trophy,
        })),
        ...(sp || []).map((project) => ({
          id: `p-${project.id}`,
          type: "project" as const,
          title: `Մեկնարկեցիր նախագիծ՝ ${project.title}`,
          sub: `${project.progress}% առաջընթաց`,
          at: project.created_at,
          icon: Rocket,
        })),
        ...(parts || []).map((participation) => ({
          id: `j-${participation.id}`,
          type: "participation" as const,
          title: `Միացար՝ ${participation.opportunities?.title || "հնարավորությանը"}`,
          sub: participation.opportunities?.category,
          at: participation.joined_at,
          icon: Calendar,
        })),
      ...
      <div className="max-w-4xl mx-auto px-3 min-[380px]:px-4 py-6 sm:py-8 pb-32 md:pb-8">
        <header className="mb-8 animate-rise min-w-0">
          <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-primary font-semibold mb-2 max-w-full">
            <Users className="w-3.5 h-3.5 shrink-0" /> <span className="break-words">Համայնք</span>
          </div>
          <h1 className="font-display text-3xl md:text-4xl font-bold leading-tight break-words">
            Քո ակտիվությունը երիտասարդական տանը
          </h1>
          <p className="text-muted-foreground mt-2 max-w-xl break-words">
            Քրոնոլոգիական հոսք այն ամենի, ինչ արել ես մինչ այժմ՝ ձեռքբերումներ, նշաններ և միացած
            ծրագրեր։ Համընդհանուր սոցիալական հոսքը կբացվի մոդերացիայի ներդրումից հետո։
          </p>
        </header>
                ...
                <div className="card-interactive p-3 min-[380px]:p-4 min-w-0 overflow-hidden">
                  <div className="flex flex-col min-[420px]:flex-row min-[420px]:items-start justify-between gap-2 min-w-0">
                    <div className="min-w-0">
                      ...
                      {it.sub && (
                        <p className="text-xs text-muted-foreground mt-0.5 capitalize break-words">
                          {it.sub}
                        </p>
                      )}
                    </div>
                    <time className="text-[11px] text-muted-foreground shrink-0 break-words">
                      {formatDistanceToNow(new Date(it.at), { addSuffix: true, locale: hy })}
                    </time>
                  </div>
                </div>
              </li>
