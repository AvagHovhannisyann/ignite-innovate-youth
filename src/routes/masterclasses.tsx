...
function isMasterclass(opportunity: Tables<"opportunities">) {
  const hay = `${opportunity.category || ""} ${(opportunity.tags || []).join(" ")}`.toLowerCase();
...
function Masterclasses() {
  ...
  const [items, setItems] = useState<Tables<"opportunities">[] | null>(null);
      ...
      <div className="max-w-5xl mx-auto px-3 min-[380px]:px-4 py-6 sm:py-8 pb-32 md:pb-8">
        <header className="mb-8 animate-rise min-w-0">
          <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-primary font-semibold mb-2 max-w-full">
            ...
            <span className="break-words">Մաստեր-դասեր</span>
          </div>
          <h1 className="font-display text-3xl md:text-4xl font-bold leading-tight break-words">
            Սովորիր մասնագետներից
          </h1>
          <p className="text-muted-foreground mt-2 max-w-xl break-words">
            Գործնական դասեր՝ մենթորների և հրավիրյալ մասնագետների կողմից։ Գրանցվիր՝ տեղ ապահովելու
            համար։ Բոլոր դասերը անդամների համար անվճար են։
          </p>
        </header>
            ...
            {items.map((o) => (
              <article key={o.id} className="card-interactive p-4 sm:p-5 min-w-0 overflow-hidden">
                <div className="flex items-start justify-between gap-3 mb-2 min-w-0">
                  ...
                  {o.difficulty && (
                    <span className="max-w-[38%] text-[11px] uppercase tracking-normal px-2 py-0.5 rounded-full bg-secondary text-secondary-foreground shrink-0 break-words text-center">
                      {o.difficulty}
                    </span>
                  )}
                </div>
                {o.description && (
                  <p className="text-sm text-muted-foreground break-words">{o.description}</p>
                )}
                <div className="flex flex-wrap gap-3 mt-4 text-xs text-muted-foreground">
                  {o.date && (
                    <span className="inline-flex items-center gap-1 break-words">
                      <Calendar className="w-3.5 h-3.5 shrink-0" />{" "}
                      {new Date(o.date).toLocaleDateString("hy-AM")}
                    </span>
                  )}
                  {o.duration && (
                    <span className="inline-flex items-center gap-1 break-words">
                      <Clock className="w-3.5 h-3.5 shrink-0" /> {o.duration}
                    </span>
                  )}
                </div>
              </article>
