...
function Achievements() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setProfile(p);
      setEarned((a || []).map((achievement) => achievement.badge));
    })();
  }, [user, loading, nav]);
      ...
      <div className="max-w-5xl mx-auto px-3 min-[380px]:px-4 sm:px-6 py-6 sm:py-8 pb-32 md:pb-8">
        <header className="mb-8 animate-rise min-w-0">
          <div className="inline-flex items-center gap-2 text-xs uppercase tracking-widest text-primary font-semibold mb-2 max-w-full">
            ...
            <span className="break-words">Ձեռքբերումներ</span>
          </div>
          <h1 className="font-display text-2xl min-[380px]:text-3xl md:text-4xl font-bold leading-tight break-words">
            Նշաններ, մակարդակներ և քայլեր
          </h1>
          <p className="text-muted-foreground mt-2 max-w-xl break-words">
            Ստացիր նշաններ՝ մասնակցելով, սկսելով նախագծեր և հասնելով նոր մակարդակների։ Յուրաքանչյուր
            նշան իրական աճ է քո ճանապարհի վրա։
          </p>
        </header>
        ...
        <section className="card-base p-4 sm:p-6 mb-8 animate-rise overflow-hidden">
          <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-end sm:justify-between gap-4 mb-3 min-w-0">
            <div className="min-w-0">
              ...
              <h2 className="font-display text-xl min-[380px]:text-2xl font-bold mt-1 break-words">
                {lvl.name}
              </h2>
              <p className="text-sm text-muted-foreground break-words">
                Մակարդակ {lvl.level} · {profile.xp || 0} XP
              </p>
            </div>
            {lvl.next && (
              <p className="text-sm text-muted-foreground break-words">
                {lvl.next.min - (profile.xp || 0)} XP մինչև{" "}
                <span className="text-foreground font-medium">{lvl.next.name}</span>
              </p>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
          {ALL_BADGES.map((b) => {
            ...
            return (
              <div
                key={b}
                className={`card-interactive p-4 min-[380px]:p-5 overflow-hidden min-w-0 ${has ? "" : "opacity-75"}`}
              >
                <div
                  className={`w-12 h-12 rounded-xl grid place-items-center mb-3 ${has ? "bg-gradient-hero shadow-glow" : "bg-secondary"}`}
                >
                  {has ? (
                ...
                <p className="text-xs text-muted-foreground mt-1 break-words">
                  {has ? "Ստացված է" : "Փակ է․ շարունակիր մասնակցել՝ բացելու համար"}
                </p>
              </div>
            );
          })}
