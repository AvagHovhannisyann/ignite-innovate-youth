...
function friendlyError(msg?: string | null): string {
  ...
  if (m.includes("password") && m.includes("6")) return "Գաղտնաբառը պետք է լինի առնվազն 6 նիշ։";
  if (m.includes("rate limit") || m.includes("too many"))
    return "Չափազանց շատ փորձեր։ Սպասիր մեկ րոպե և փորձիր կրկին։";
  if (m.includes("network") || m.includes("fetch"))
    return "Ցանցի խնդիր։ Ստուգիր կապը և փորձիր կրկին։";
  if (m.includes("validation") || m.includes("invalid email")) return "Մուտքագրիր ճիշտ էլ. հասցե։";
  return msg;
}
...
function AuthPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!/^\S+@\S+\.\S+$/.test(trimmedEmail)) {
      setError("Մուտքագրիր ճիշտ էլ. հասցե։");
      return;
    }
    ...
    try {
      if (mode === "signup") {
        if (!fullName.trim()) {
          setError("Մուտքագրիր անունդ և ազգանունդ։");
          setLoading(false);
          return;
        }
  ...
  async function googleSignIn() {
    ...
    setLoading(true);
    try {
      // Native Supabase OAuth; the /auth/callback route completes the session.
  ...
  return (
    <div className="min-h-dvh bg-gradient-soft overflow-x-hidden">
      ...
      <div className="max-w-md lg:max-w-6xl mx-auto px-3 min-[380px]:px-4 sm:px-6 py-8 sm:py-12 pb-28 md:pb-12 lg:grid lg:grid-cols-2 lg:gap-12 xl:gap-16 lg:items-stretch">
        ...
        <div className="w-full lg:max-w-md lg:justify-self-center lg:self-center">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl min-[380px]:text-3xl font-bold leading-tight break-words">
              {isForgot
                ? "Վերականգնիր գաղտնաբառը"
                : isSignup
                  ? "Միացիր Երիտասարդական Տանը"
                  : "Բարի վերադարձ"}
            </h1>
            <p className="text-muted-foreground text-sm mt-2 break-words">
              {isForgot
                ? "Մենք կուղարկենք անվտանգ վերականգնման հղում։"
                : isSignup
                  ? "Ստեղծիր հաշիվդ՝ բացահայտելու սկսելու համար։"
            ...
            <>
              ...
              <div className="relative mb-4">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="bg-gradient-soft px-2 text-muted-foreground text-center break-words">
                    կամ {isSignup ? "գրանցվիր" : "մուտք գործիր"} էլ. հասցեով
                  </span>
                </div>
              </div>
            </>
          ...
          <form
            onSubmit={submit}
            className="bg-card border border-border rounded-2xl p-4 min-[380px]:p-6 shadow-soft space-y-4 overflow-hidden min-w-0"
          >
            {isSignup && (
              <div>
          ...
          <div className="text-center text-sm text-muted-foreground mt-6 space-y-2">
            ...
            <p className="text-xs">
              <Link
                to="/"
                className="inline-flex items-center justify-center min-h-[44px] px-3 hover:text-foreground"
              >
                ← Տուն վերադառնալ
              </Link>
            </p>
